document.addEventListener("DOMContentLoaded", function () {
    ...
    if (id && unique_id) {
        // Assuming your uniqueid div has a child span for the label and then the ID value
        const uniqueIdLabelSpan = unique_id.querySelector('.uniqueid-label');
        if (uniqueIdLabelSpan) {
            uniqueIdLabelSpan.nextSibling.textContent = " " + id; // Update the text content directly next to the label
        } else {
            unique_id.innerText = "ID: " + id; // Fallback if label is not present
        }
    }


    // The following functionalities (feature cards, join quiz, testimonial slider, smooth scrolling, try it now, cta buttons)
    // seem to be general page functionalities, not specific to the header.
    // They are left here for completeness but ideally, they should be in a separate `main.js` or `app.js` file.
